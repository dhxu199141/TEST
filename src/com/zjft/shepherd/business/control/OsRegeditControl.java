...
public class OsRegeditControl {
	private static Log log = LogFactory.getLog(OsRegeditControl.class);
	private static final String cmdId = "200033";
	...
	public Map<String, Object> remoteControl(Map<String, Object> pHashMap) {
		...
		//获取注册表操作选项
		String regOption = (String) pHashMap.get("regOption");//0：关闭，1：开启
		String content = null;
		if(regOption != null && regOption.equals("1"))
		{
			content = "开启";
		}
		else
		...
		try
		{
			for(int i = 0; i < devInfo.length ; i++)
			{
				...
				//boolean commResult = SocketUtil.sendCmdToRvcMutil(genOutXml(tempRemote[0], tempRemote[1], regOption), tempRemote[1], "50006", retBody, 20, 60, Integer.valueOf(tempRemote[2].toString()), Integer.valueOf(tempRemote[3].toString()));
				//增加参数设备号，通过消息转发，获取设备的加密密钥，并进行转发    2011-8-22  mod by cy
				boolean commResult = MessageEncoded.sendCmdToRvcMutil(genOutXml(tempRemote[0], tempRemote[1], regOption), tempRemote[1], "50006", retBody, 20, 60, Integer.valueOf(tempRemote[2].toString()), Integer.valueOf(tempRemote[3].toString()), tempRemote[0]);
				boolean controlResult = false;
				StringBuffer resultContent = new StringBuffer();
				if(commResult)
				{
					String retcode = XmlUtil.getValue(retBody.toString(), "//root/retcode", "value");
					resultContent.append(content).append("设备").append(tempRemote[0]).append("注册表权限");
				...
				saveRemoteTrace(cmdId, pHashMap.get("user").toString(), tempRemote[0], (controlResult ? "成功" : "失败"), content+"注册表权限");
			}
		...
		catch(Exception e)
		{
			...
			saveRemoteTrace(cmdId, pHashMap.get("user").toString(), tempRemote[0], "失败", content+"注册表权限");
	...
	private String genOutXml(String devNo, String ip, String regOption)
	{
		StringBuffer xml = new StringBuffer();
		xml.append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
		xml.append("<root>");
		xml.append("<cmdid value=\"200033\"/>");
		...
		xml.append("<actioninfo regoption=\"").append(regOption).append("\"/>");
