	...
	public Map<String, Object>  remoteControl(Map<String, Object> pHashMap) {
		...
		try
		{
			...
			if(commResult)
			{
				...
				resultContent.append("修改设备").append(devNo).append("上用户").append(userName).append("密码");
			...
			saveRemoteTrace(cmdId, pHashMap.get("user").toString(), devNo,(controlResult ? "成功" : "失败"), "修改密码");
		...
		catch(Exception e)
		{
			...
			saveRemoteTrace(cmdId, pHashMap.get("user").toString(), devNo, "失败", "修改密码");
